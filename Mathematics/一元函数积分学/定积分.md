...
### 计算
##### 牛顿-莱布尼兹公式
设函数$F(x)$是连续函数$f(x)$在$[a,b]$上的一个原函数，则$$\displaystyle\int_a^bf(x)\mathrm dx=F(x)\bigg|_a^b=F(b)-F(a)$$
###### 推广
1. 设函数$f(x)$在$[a,b]$上有原函数$F(x)$，则$\displaystyle\int_a^bf(x)\mathrm dx=F(b)-F(a)$
2. 设函数$f(x)$在$[a,b]$上分段有原函数$F(x)$，若每一段定积分都存在，那么定积分的结果是每一段定积分的和，若至少有一个不存在，那么整个定积分发散
##### 定积分的一些结论
1. 若$f(x)$为连续的偶函数，那么：$$\displaystyle\int_{-a}^af(x)\mathrm dx=2\int_0^af(x)\mathrm dx$$
2. 若$f(x)$为连续的奇函数，那么：$$\displaystyle\int_{-a}^af(x)\mathrm dx=0$$
3. 若$f(x)$是以$T$为周期的连续函数，对任意实数$a$，有$$\displaystyle\int_a^{a+T}f(x)\mathrm dx=\int_0^Tf(x)\mathrm dx$$
4. 若$f(x)$为连续函数，那么：$$\displaystyle\int_a^bf(x)\mathrm dx=\int_a^bf(a+b-x)\mathrm dx$$
5. 华莱士公式（点火公式）
	- 积分区间在$\displaystyle[0,\frac\pi 2]$上： $$\displaystyle\int_0^{\frac\pi 2}\sin^nx\mathrm dx=\int_0^{\frac\pi 2}\cos^nx\mathrm dx=\begin{cases}\displaystyle\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdot\text{...}\cdot\frac 23\cdot 1& n\text{为大于1的奇数}\\[2ex]\displaystyle\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdot\text{...}\cdot\frac 12\cdot \frac\pi 2 & n\text{为正偶数}\end{cases}$$
	- 积分区间在$[0,\pi]$上： $$\displaystyle\int_0^\pi\sin^nx\mathrm dx=\begin{cases}\displaystyle 2\cdot\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdot\text{...}\cdot\frac 23\cdot 1& n\text{为大于1的奇数}\\[2ex]\displaystyle2\cdot\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdot\text{...}\cdot\frac 12\cdot \frac\pi 2 & n\text{为正偶数}\end{cases}$$$$\displaystyle\int_0^\pi\cos^nx\mathrm dx=\begin{cases}\displaystyle 0& n\text{为正奇数}\\[2ex]\displaystyle2\cdot\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdot\text{...}\cdot\frac 12\cdot \frac\pi 2 & n\text{为正偶数}\end{cases}$$
	- 积分区间在$[0,2\pi]$上：$$\displaystyle\int_0^{2\pi}\sin^nx\mathrm dx=\int_0^{2\pi}\cos^nx\mathrm dx=\begin{cases}\displaystyle 0& n\text{为正奇数}\\[2ex]\displaystyle4\cdot\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdot\text{...}\cdot\frac 12\cdot \frac\pi 2 & n\text{为正偶数}\end{cases}$$
6. $\Gamma$函数
	- 定义：$\displaystyle\Gamma(\alpha+1)=\int_0^{+\infty}x^\alpha e^{-x}\mathrm dx$
	- 初值：$\begin{cases}\displaystyle\Gamma(1)=1\\[2ex]\displaystyle\Gamma\left(\frac{1}{2}\right)=\sqrt\pi\end{cases}$
	- 递推式：$\Gamma(\alpha+1)=\alpha\cdot\Gamma(\alpha)$，若$\alpha$是整数，那么$\Gamma(\alpha+1)=\alpha!$
	- 例：$$\displaystyle\int_0^{+\infty}e^{-x^2}\mathrm dx=\int_0^{+\infty}\frac{x}{x}\cdot e^{-x^2}\mathrm dx=\frac{1}{2}\int_0^{+\infty}(x^2)^{-\frac{1}{2}}e^{-x^2}\mathrm d(x^2)=\frac{1}{2}\Gamma\left(\frac{1}{2}\right)=\frac{\sqrt{\pi}}{2}$$$$\displaystyle\int_0^{+\infty}xe^{-x^2}\mathrm dx=\frac{1}{2}\displaystyle\int_0^{+\infty}e^{-x^2}\mathrm d(x^2)=\frac{1}{2}\Gamma(1)=\frac{1}{2}$$