...
注意瑕点的问题
##### $\Gamma$函数
1. 定义：$$\displaystyle\Gamma(\alpha)=\int_0^{+\infty}x^{\alpha-1}\mathrm e^{-x}\mathrm dx\xlongequal{x=t^2}2\int_0^{+\infty}t^{\alpha+1}\mathrm e^{-t^2}\mathrm dt$$
2. 递推式（分部积分法推导）：$$\displaystyle\Gamma(\alpha+1)=\int_0^{+\infty}x^\alpha\mathrm e^{-x}\mathrm dx=x^\alpha\cdot(-\mathrm e^x)\bigg|_0^{+\infty}+a\int_0^{+\infty}x^{\alpha-1}\mathrm e^{-x}\mathrm dx=a\Gamma(\alpha)$$
3. $\Gamma(1)=1$、$\displaystyle\Gamma\left(\frac 12 \right)=\sqrt\pi$
